...
final class MigrationGenerator extends AbstractGenerator
{
    ...
    private function buildBelongsToManyColumns(Model $model, BelongsToMany $belongsToMany): string
    {
        $columns = [];

        foreach ($belongsToMany->models($model) as $segment) {
            $columns[] = \sprintf(
    ...
    private function buildMorphedByManyColumns(MorphedByMany $morphedByMany): string
    {
        $columns = [];

        $columns[] = \sprintf(
            '$table->unsignedBigInteger(\'%s_id\');',
            Str::snake($morphedByMany->related()),
        );

        $columns[] = \sprintf(
            '$table->morphs(\'%s\');',
            Str::lower(Str::singular($morphedByMany->related()).'able'),
        );
