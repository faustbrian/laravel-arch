...
final readonly class StrMixin
{
    ...
    {
        return function (string $content, int $times = 4, string $type = 'space'): string {
    ...
    public function appPath(): Closure
    {
        return function (?string $path = null): string {
            return \trim(\sprintf('%s/%s', \str_replace('\\', '/', Config::get('arch.app_path')), $path));
        };
    }
        ...
        return function (string $statement): array {
            ...
            foreach (\explode(' ', $statement) as $key => $part) {
                ...
                if (\count($partComponents) === 2) {
                    if (isset($result[$partComponents[0]])) {
